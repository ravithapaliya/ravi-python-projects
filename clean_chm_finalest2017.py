...
base_output_folder = r"C:\Users\ravit\Documents\tree research work new\Batch_Results2017_1m_Final"
...
minimum_tree_height = 6.56              # about 2 meters
...
for filename in las_files:
    ...
    out_chm = os.path.join(tile_folder, f"{tile_id}_CHM_1m_2017.tif")
    out_shp = os.path.join(tile_folder, f"{tile_id}_tree_crowns_1m_2017.shp")
    ...
    try:
        ...
        excluded_classes = noise_classes + building_classes + water_classes + breakline_classes + road_classes + ignored_classes
        canopy_m = np.isin(cls, vegetation_classes) & (~np.isin(cls, excluded_classes))
        ...
        if np.sum(canopy_m) < min_points_required:
            print(f"  Skipping. Too few vegetation points after filtering: {np.sum(canopy_m)}")
            continue
        ...
        if np.sum(canopy_valid) < min_points_required:
            print("  Skipping. Too few vegetation points after sanity filter.")
            continue
