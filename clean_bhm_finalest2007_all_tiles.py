...
base_output_folder = r"C:\Users\ravit\Documents\building_height_results_2007"
...
# 2007 classes actually present in your tiles
ground_classes = [2]
primary_roof_classes = [1]
noise_classes = [7]

max_surface_gap_ft = 5.0
...
print("Loading 2007 building footprints...")
...
for filename in las_files:
    ...
    out_bhm = os.path.join(tile_folder, f"{tile_id}_BHM_1m_2007.tif")
    out_buildings = os.path.join(tile_folder, f"{tile_id}_building_heights_2007.shp")
    ...
    try:
        ...
        broad_surface_m = (~np.isin(cls, noise_classes)) & (~np.isin(cls, ground_classes))
...
if len(all_tile_records) == 0:
    ...
    out_all = os.path.join(base_output_folder, "Meck2007_BldgHt_All.shp")
    buildings_final.to_file(out_all)

    valid_only = buildings_final[buildings_final["h_status"].isin(["ok", "fb_recov"])].copy()
    out_valid = os.path.join(base_output_folder, "Meck2007_BldgHt_Valid.shp")
    valid_only.to_file(out_valid)

    fallback_only = buildings_final[buildings_final["h_status"] == "fb_recov"].copy()
    out_fallback = os.path.join(base_output_folder, "Meck2007_BldgHt_Fallback.shp")
