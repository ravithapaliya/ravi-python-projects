...
base_folder = r"F:\MYPHDRESEARCHOUTPUTS\Batch_Results2023_1m_Final"
...
output_folder = r"F:\MYPHDRESEARCHOUTPUTS\Countywide_Combined_Results_2023\QOL_NPA_Min2m_Outputs"
os.makedirs(output_folder, exist_ok=True)

county_output_folder = r"F:\MYPHDRESEARCHOUTPUTS\Countywide_Combined_Results_2023"
...
chm_files = sorted(glob.glob(os.path.join(base_folder, "**", f"*_CHM_1m_{year}.tif"), recursive=True))
print(f"Found {len(chm_files)} CHM rasters.")

if len(chm_files) == 0:
    raise FileNotFoundError("No CHM rasters found.")

print("Finding crown shapefiles...")
crown_files = sorted(glob.glob(os.path.join(base_folder, "**", f"*_tree_crowns_1m_{year}.shp"), recursive=True))
...
processed_ids_in_summary = set()
if len(summary_rows) > 0:
    processed_ids_in_summary = set(
        int(r["NPA_ID"]) for r in summary_rows
        if "NPA_ID" in r and pd.notna(r["NPA_ID"])
    )
...
for idx, npa in qol.iterrows():
    ...
    out_chm = os.path.join(npa_folder, f"NPA_{npa_id}_CHM_CanopyOnly_NoBuildings_Min2m_{year}.tif")
    out_crowns = os.path.join(npa_folder, f"NPA_{npa_id}_tree_crowns_NoBuildings_Min2m_{year}.shp")
    ...
    if len(crown_gdfs) > 0:
        crowns_all = gpd.GeoDataFrame(pd.concat(crown_gdfs, ignore_index=True), crs=raster_crs)
...
if len(npa_crown_files) == 0:
    ...
    if len(crown_list) > 0:
        county_crowns = gpd.GeoDataFrame(pd.concat(crown_list, ignore_index=True), crs=raster_crs)
